# -*- coding: utf-8 -*-
"""250704_NVHpowersourceEDA_V01.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1O7mr4WJu9pJqJ7I5y9aONwkn1jl9EiNw
"""
    ...
    if merged_df_list:
        ...
        for stat1 in merged_df['Stat1'].dropna().unique():
            with st.expander(f"Stat1: {stat1}"):
                ...
                for stat2 in filtered_df['Stat2'].dropna().unique():
                    stat2_df = filtered_df[filtered_df['Stat2'] == stat2]
                    if agg_option == "SUM":
                        agg_df = stat2_df.groupby('Date')['Value'].sum().reset_index()
                    else:
                        agg_df = stat2_df.groupby('Date')['Value'].mean().reset_index()
                    fig = px.bar(agg_df, x='Date', y='Value', title=f"{stat2} ({agg_option})")
